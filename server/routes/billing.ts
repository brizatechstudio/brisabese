...
const idempotencyKey=(req:BillingRequest)=>{const value=String(req.headers['idempotency-key']||'').trim();if(!value)return undefined;if(!/^[A-Za-z0-9._:-]{8,200}$/.test(value))throw new Error('Invalid Idempotency-Key.');return value;};
const fail=(res:express.Response,error:unknown)=>{const message=error instanceof Error?error.message:'Billing operation failed.';const status=/not found/i.test(message)?404:/not configured|disabled/i.test(message)?503:/invalid|required|available|origin|partial|too long|idempotency/i.test(message)?400:500;res.status(status).json({error:{code:'BILLING_ERROR',message:process.env.NODE_ENV==='production'&&status>=500?'Billing operation failed.':message}});};
...
billingRouter.post('/api/billing/checkout',async(req,res)=>{try{const request=req as BillingRequest;const key=idempotencyKey(request);res.status(201).json(await localBillingEngine.createCheckout(ctx(request),{...(req.body||{}),planId:planId(req.body?.planId),checkoutUrl:boundedString(req.body?.checkoutUrl,2048,'Checkout URL'),idempotencyKey:key}));}catch(error){fail(res,error);}});
...
billingRouter.post('/api/billing/subscription/cancel',async(req,res)=>{try{const request=req as BillingRequest;const key=idempotencyKey(request);res.json(await localBillingEngine.cancelSubscription(ctx(request),Boolean(req.body?.immediate),key));}catch(error){fail(res,error);}});
...
billingRouter.post('/api/billing/invoices/:invoiceId/refund',async(req,res)=>{try{const request=req as BillingRequest;const key=idempotencyKey(request);const invoiceId=String(req.params.invoiceId||'');if(!/^[A-Za-z0-9_-]{3,160}$/.test(invoiceId))throw new Error('Invalid invoice id.');const amount=req.body?.amountCents===undefined?undefined:Number(req.body.amountCents);if(amount!==undefined&&!Number.isSafeInteger(amount))throw new Error('Invalid refund amount.');res.status(201).json(await localBillingEngine.refund(ctx(request),invoiceId,amount,boundedString(req.body?.reason,255,'Refund reason'),key));}catch(error){fail(res,error);}});
